...
export function getChatSessionId(siteKey = ADAMES_SITE_KEY): string | null {
  if (typeof sessionStorage === 'undefined') return null;
  return sessionStorage.getItem(sessionStorageKey(siteKey));
}

export function setChatSessionId(id: string, siteKey = ADAMES_SITE_KEY): void {
...
export async function fetchChatSiteConfig(siteKey: string): Promise<ChatSiteConfig | null> {
  const res = await fetch(
    `${NEXRENA_API_URL}/api/chat/config?siteKey=${encodeURIComponent(siteKey)}`,
  );
...
export async function sendChatMessage(
  ...
  const siteKey = options?.siteKey ?? ADAMES_SITE_KEY;
  const res = await fetch(`${NEXRENA_API_URL}/api/chat`, {
    ...
    body: JSON.stringify({
      ...
      pageUrl:
        options?.pageUrl ?? (typeof window !== 'undefined' ? window.location.pathname : undefined),
    }),
  });
...
export async function submitChatIntake(payload: {
  ...
  const siteKey = payload.siteKey ?? ADAMES_SITE_KEY;
  const res = await fetch(`${NEXRENA_API_URL}/api/chat`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
